...
def conv_net(x):
    ...
    pool2 = tf.layers.max_pooling2d(conv1, 3, 3)

    conv3 = tf.layers.conv2d(pool2, 128, 4, activation=tf.nn.relu)
    # Flatten the data to a 1-D vector for the fully connected layer
    pool3_flat = tf.contrib.layers.flatten(conv3)

    # Fully connected layer of width N
    fc1 = tf.layers.dense(pool3_flat, 1000)
    fc2 = tf.layers.dense(fc1, 500)
    fc3 = tf.layers.dense(fc2, 250)
    fc3 = tf.layers.dense(fc2, 250)
...
def my_vanilla_neural_net(x):
    ...
    fc1 = tf.layers.dense(x, 1500, activation=tf.nn.relu)
